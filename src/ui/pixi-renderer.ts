...
export interface PixiEventHandlers {
  ...
  onFloatingComponentDragStart?: (
    floatingComponentId: string,
    globalX: number,
    globalY: number
  ) => void;
  onFloatingComponentLegDragStart?: (
    floatingComponentId: string,
    legIndex: number,
    globalX: number,
    globalY: number
...
export class PixiRenderer {
  ...
  async init(container: HTMLElement, handlers: PixiEventHandlers = {}): Promise<void> {
    ...
    this.breadboardContainer.position.set(
      PixiRenderer.LABEL_PADDING_X,
      PixiRenderer.LABEL_PADDING_Y
    );
    this.connectionsContainer.position.set(
      PixiRenderer.LABEL_PADDING_X,
      PixiRenderer.LABEL_PADDING_Y
    );
    this.componentsContainer.position.set(
      PixiRenderer.LABEL_PADDING_X,
      PixiRenderer.LABEL_PADDING_Y
    );
    this.voltageOverlayContainer.position.set(
      PixiRenderer.LABEL_PADDING_X,
      PixiRenderer.LABEL_PADDING_Y
    );
    ...
    this.errorOverlayContainer.position.set(
      PixiRenderer.LABEL_PADDING_X,
      PixiRenderer.LABEL_PADDING_Y
    );
  ...
  private renderBreadboardSubstrate(): void {
    ...
    plasticSurface.rect(
      ...
      0,
      (BreadboardLayout.STRIP_RIGHT_END - BreadboardLayout.STRIP_LEFT_START + 1) *
        PixiRenderer.HOLE_SPACING,
      height
    );
    ...
    // Right rail area
    plasticSurface.rect(
      BreadboardLayout.RAIL_RIGHT_POSITIVE * PixiRenderer.HOLE_SPACING,
      0,
    ...
    leftNegLabel.x =
      BreadboardLayout.RAIL_LEFT_NEGATIVE * PixiRenderer.HOLE_SPACING +
      PixiRenderer.HOLE_SPACING / 2;
    leftNegLabel.y = -8;
    ...
    rightNegLabel.x =
      BreadboardLayout.RAIL_RIGHT_NEGATIVE * PixiRenderer.HOLE_SPACING +
      PixiRenderer.HOLE_SPACING / 2;
    rightNegLabel.y = -8;
   ...
   */
  private renderHole(pos: Position, holeColor: number, isOccupied: boolean = false): Graphics {
    ...
    hole.circle(
      pixels.x - highlightOffset,
      pixels.y - highlightOffset,
      PixiRenderer.HOLE_VISUAL_RADIUS / 3
    );
  ...
  renderConnections(
    reteManager: {
      ...
      getComponentNode(
        componentId: string
      ): { componentId: string; componentType: ComponentType } | null;
    ...
    for (const connection of connections) {
      ...
      for (const component of components) {
        for (let i = 0; i < component.positions.length - 1; i++) {
          ...
          line.bezierCurveTo(p1.x, p1.y - offset / 2, p2.x, p2.y - offset / 2, p2.x, p2.y);
          ...
          line.hitArea = {
            contains: (x: number, y: number) => {
              ...
              const distToSegment = this.distanceToLineSegment({ x, y }, p1, p2);
  ...
  private renderComponent(
    ...
    if (isDragging) {
      // Original (faded)
      const original = this.renderComponentGraphics(
        component,
        component.positions,
        simulation,
        positionToNode
      ...
      if (dragState.previewPositions) {
        const preview = this.renderComponentGraphics(
          component,
      ...
      const graphics = this.renderComponentGraphics(
        component,
        component.positions,
        simulation,
        positionToNode
  ...
  private createRotationHandle(
    ...
    for (let angle = arrowStartAngle; angle <= arrowEndAngle; angle += 0.1) {
      arrowGraphics.lineTo(Math.cos(angle) * arrowRadius, Math.sin(angle) * arrowRadius);
    }
    ...
    handleContainer.position.set(centerPixels.x + handleOffsetX, centerPixels.y + handleOffsetY);
   ...
   */
  private createComponentHitArea(
    component: AnyComponent
  ): { contains: (x: number, y: number) => boolean } | null {
      ...
      case ComponentType.WIRE: {
        ...
        return {
          contains: (x: number, y: number) => {
            ...
            if (
              Math.abs(x - p1.x) < hitTolerance &&
              y >= Math.min(p1.y, midY) &&
              y <= Math.max(p1.y, midY)
            ) {
              return true;
            }
            ...
            if (
              Math.abs(x - p2.x) < hitTolerance &&
              y >= Math.min(midY, p2.y) &&
              y <= Math.max(midY, p2.y)
            ) {
              return true;
            }
      ...
      case ComponentType.LED: {
        ...
        const bodyHeight =
          component.type === ComponentType.RESISTOR
            ...
            : PixiRenderer.LED_BODY_RADIUS;
        const bodyRadius = component.type === ComponentType.LED ? PixiRenderer.LED_BODY_RADIUS : 0;
   ...
   */
  private renderStraightResistor(
    graphics: Graphics,
    component: Resistor,
   ...
   */
  private calculateCenterPosition(positions: Array<{ row: number; col: number }>): {
    row: number;
    col: number;
  } {
  ...
  private createErrorIcon(error: CircuitError, x: number, y: number): Container {
    ...
    // Symbol
    const symbol =
      error.type === ErrorType.SHORT_CIRCUIT
        ? '✕'
