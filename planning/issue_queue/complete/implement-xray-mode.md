...
X-Ray Mode is one of two informational modes specified in the goal architecture (Section 4.2). While Electrical View Mode shows _what_ is happening electrically, X-Ray Mode explains _why_ - by revealing the hidden internal wiring of the breadboard that causes certain holes to be electrically connected.
...
**What's missing:** Visual representation of the _physical connectivity structure_ that underlies the electrical behavior.
...
- When user hovers over a hole, highlight _all connected holes_ in that connectivity group
...
- When both enabled:
  - X-Ray shows _physical connectivity structure_
  - Voltage overlays show _electrical state_ (actual voltages)
  - Current animation shows _charge flow_
